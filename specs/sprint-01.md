...
- **pgTAP corrido pelo orquestrador: 85/85 esperado após correcções.** `supabase/tests/0014_haccp.sql` (`plan(85)`, relógio controlado) foi corrido pelo orquestrador em Postgres 16 com pgTAP 1.3 e o stub Supabase, sobre a migração 0029 aplicada limpa sobre as 28 anteriores. Foram detectados e corrigidos 5 defeitos pequenos (todos no teste, mais um `id` ambíguo na RPC `haccp_record_temperature`): (1) referência não qualificada a `id` na resolução do restaurante dentro da RPC — passou a `cp.id`; (2) sete literais de UUID com caracteres fora de `0-9a-f` (`cp/in/mc/mi/pb/re…`) substituídos por UUIDs hexadecimais válidos e únicos; (3) verificado que não restam chamadas `perform` ao nível SQL (já estavam como `select`); (4) asserções 23 e 29 com `throws_ok` de 2 argumentos convertidas para a forma de 4 argumentos (`P0001/haccp_ponto_inexistente` e `42501/null`); (5) grupo "retenção / purga" a ler o default de 24 meses com role errado (RLS escondia o restaurante A) — resolvido com `reset role` antes da asserção 67 e `set local role authenticated` antes do teste de escrita da cozinha. Os gates de web (`pnpm typecheck`, `pnpm build`, `pnpm test` com 135 testes, todos verdes) foram executados e confirmam que `apps/web` não foi tocado.
